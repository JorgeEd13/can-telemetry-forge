...
<p align="center">
  <img src="https://img.shields.io/badge/status-F1%20signal%20model-orange" alt="Status: F1 signal model">
...
> ⚠️ **Honest status:** the **J1939-grounded signal model (F1) is built** — the 11
> Tier-1 signals are generated by deterministic, seeded per-signal functions with
> documented cross-signal correlations, gated by CAN capability era, and clamped to
> their published J1939 ranges; see [`docs/DATA_DICTIONARY.md`](docs/DATA_DICTIONARY.md).
> The installable package, `forge` CLI, offline tests and CI (Linux + Windows) are
> in place. Still building phase by phase (see [`docs/ROADMAP.md`](docs/ROADMAP.md)):
> the **fleet simulator + writers (F2)** turn the signal model into a one-command
> dataset — that's when the usage section below goes live.
